...
let sortable = null; // 추가된 변수
let hasOrderChanged = false; // 추가된 변수
let originalMenuOrder = []; // 추가된 변수
...
function fn_menuList(action = null) {
    ...
    $.ajax({
        url: '/admin/menu_mng/sub_menu_mng_list',
        method: 'POST',
        dataType: 'json',
        ...
        success: function (data) {
            const subMenuList = data.subMenuList || [];
            ...
            subMenuList.forEach(function (item, index) {
				...
				<tr class="menu-row" data-seq="${item.subMenuSeq}" data-order="${item.subMenuOrder || index + 1}">
				    <td ${redStyle}>
				        <span class="order-display">${(pageIndex - 1) * pageSize + index + 1}</span>
				    </td>
				    <td ${redStyle}>${item.topMenuNm || ''}</td>
				    <td ${redStyle}>${item.subMenu || ''}</td>
				    <td ${redStyle}>${item.subMenuNm || ''}</td>
				    <td>
				        <button type="button" class="btn btn-outline-primary btn-sm me-1" onclick="fn_menuLink('menuUpdate', '${item.subMenuSeq}')">수정</button>
				        ${
				            item.deleteYn === 'Y'
				                ? `<button type="button" class="btn btn-outline-success btn-sm" onclick="fn_activeMenu('${item.subMenuSeq}', '${item.subMenuNm}','Y')">활성화</button>`
				                : `<button type="button" class="btn btn-outline-danger btn-sm" onclick="fn_activeMenu('${item.subMenuSeq}', '${item.subMenuNm}','N')">비활성화</button>`
				        }
				    </td>
				</tr>
                `;
...
function fn_menuLink(_page,seq){
	...
	if(_page=='menuRegist'){
		url += '/sub_menu_create';
	}else if(_page =='subMenuList'){
		url += '/menu_mng';
		url += '/sub_menu_mng';
	}else if(_page == 'menuUpdate'){
		url += '/sub_menu_update';
		url += '?sub_menu_seq='+seq
	}
