...
$(document).ready(function () {
	...
	$(document).on('click', '.selectBoard', function() {
	    ...
	    const targetTable = $(this).data('targettable');
		console.log(targetTable);
	    fn_menuLink('boardDetail', subMenu, topMenu, targetTable);
	});
});


function fn_menuLink(_page, subMenu, topMenu, targetTable) {
	url = '/user';	
	if (_page == 'boardDetail') {
	    url += '/' + topMenu + '/' + subMenu + '/data_board_view';
	    if (targetTable) {
		...
		success: function(data){
			...
			list.forEach(function(item, index) {
				...
				<tr>
					...
					<a href="javascript:void(0);" class="text-decoration-none text-dark fw-semibold selectBoard" data-submenu="${subMenu}" data-topmenu="${topMenu}" data-targettable="${item.targetTable}">
					    ${item.apiName || '제목 없음'}
					</a>
				    </td>
				</tr>
